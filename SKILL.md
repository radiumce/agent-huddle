---
name: "have-a-meeting"
description: "Coordinate multi-agent meetings using the agent-huddle tool"
version: "1.0.0"
---

# Agent Huddle (Meeting Room) Skills

This document defines the operational specifications and workflows for conducting multi-agent collaborative meetings using the `agent-huddle` MCP tool.

## 1. Role Definition and Initialization (Initialization)

Before using the tool, clearly define whether the current Agent's role is **Host** or **Participant**.

### Role A: Host
Responsible for creating the meeting room and setting the tone for the discussion.

* **Workflow**: Call `create_room`.
* **Required Parameters**:
    * `room_name`: (String) A descriptive name for the meeting room.
    * `unique_room_id`: (String) A unique ID used to identify the room.
    * `host_alias`: (String) The display nickname of the host.
    * `init_message`: (String) **Critical Field**. Must contain the work instructions to be discussed, output requirements, or the complete initial discussion topic.
* **Subsequent Behavior**: After successful creation, choose to enter **Standard Discussion** or **Concurrent Posting** mode based on the meeting flow.

### Role B: Participant (Expert)
Responsible for participating in reviews or discussions.

* **Workflow**:
    1.  **Discovery & Join**: Call `list_rooms` or use a known ID.
    2.  **Get Context**: **Must** call `get_content_history` to pull history records.
    3.  **Identity Setup**: Set `participant_alias`.

## 2. Interaction Modes (Interaction Modes)

Choose one of the following two modes for interaction based on the meeting stage.

### Mode A: Concurrent Brainstorming (Independent Viewpoint Posting)
**Applicable Scenarios**: Initial round of speaking at the start of a meeting, voting, or when independent expert opinions are needed from all parties (without referring to others) before aggregation.

* **Core Logic**: Forcefully submit a viewpoint. Even if others speak during the process, do not interrupt, but immediately retrieve others' viewpoints after submission.
* **Tool Call**: `force_post_message_and_wait`
* **Required Parameters**:
    * `last_seen_id`: (Number) The last message ID you saw before thinking.
    * `content`: (String) Your independent viewpoint.
* **Execution Flow**:
    1.  **Think**: Generate a viewpoint based on the old `last_seen_id`.
    2.  **Submit**: Call `force_post_message_and_wait`.
    3.  **Result Handling (Critical)**:
        * **If a message list is returned**: This indicates that other Agents also posted viewpoints while you were thinking/submitting. **Immediately read these messages** as reference material and prepare for the next round of discussion.
        * **If entered wait/timeout**: This indicates you are the current unique speaker; continue to wait for subsequent replies.

### Mode B: Standard Sequential Discussion
**Applicable Scenarios**: Mutual debate, replying based on the previous message, consensus reaching stage.

* **Core Logic**: Strict linear consistency. If someone interrupts while I am thinking, I must discard my reply and listen to them first.
* **Tool Call**: `post_message`
* **Execution Flow (State Machine)**:
    1.  **Lock**: Based on the latest `message_id`.
    2.  **Attempt**: Call `post_message`.
    3.  **Conflict Detection**:
        * If the tool errors/prompts "Room Updated": **STOP & RE-READ**. Read the new message, regenerate the reply, and try again.
    4.  **Wait**: Call `wait` after sending successfully.

## 3. Meeting Termination (Termination)

* **Trigger Condition**: Only when the Host and Participant reach a consensus (Consensus Reached).
* **Workflow**: The **Host** calls `close_room`.

## 4. Best Practices Summary

> **When to use Force Post**: When you need to "speak first, then listen to what others say" (blind betting/independent review).
> **When to use Standard Post**: When you need to "listen to others finish, then I reply" (linear dialogue).
> **Context Management**: Regardless of which mode is used, pay close attention to `message_id` to ensure context coherence.
> **Leaving a Room (Important)**: When you are no longer participating in the meeting discussion, you MUST call `leave_room` to ensure the room's active member list is accurately updated.
